...
- Refactor(describe): move describe.ts into describe/index.ts to establish a proper module entry; update internal imports for the new path. Public API and tests unchanged (imports of './describe' now resolve to the directory index).
...
- Feat(describe): derive localized weekday/month names via Luxon; options for locale/lowercase
  - Replaced static weekday table with Luxon cccc in the rule timezone.
  - Month names now honor translatorOptions.locale (LLLL), lower‑cased by default for strict style.
  - New TranslatorOptions: { locale?: string; lowercase?: boolean }.
  - Composition remains in library code; all date/time labels generated by Luxon.
  - No API breaking changes; defaults preserve current English lower‑case output.

- Perf(tests): add BENCH-gated micro-benchmarks for core algorithms
  - New: src/rrstack/perf.bounds.single-rule.test.ts
  - Scenarios: baseline active (no rules), single daily active (open end), single daily active (closed 30d), plus light benches for isActiveAt, getSegments, and classifyRange.
  - Skipped by default; enable locally with BENCH=1 (and optional ITERS).
  - Sanity section always runs minimal assertions without timing to keep CI stable.

- Fix(lint): coerce numeric template expression to string in describe/translate.strict.en.ts (restrict-template-expressions).
...
Next up (perf & profiling)

- Extend BENCH suite with additional workloads:
  - monthly “nth weekday” open/closed, yearly bymonthday, count-limited recurrences.
  - blackout overlays and tie-at-boundary cases for latest-end reverse sweep.
- Add a developer doc snippet (Handbook → Algorithms or React) with tips for running BENCH and interpreting results.
- Consider introducing a tiny-benchmark helper (e.g., tinybench) only if we need more detailed stats; otherwise keep zero-dep.
- Explore exposing a diagnostic knob (e.g., version counter or internal probe counts) solely in test builds to attribute hotspots if needed.
