...
- Bounds & descriptions (America/Chicago daily 1‑day rule):
  ...
  - Earliest pre‑pass refinement: use compiled `dtstart` as the base for
    `rrule.after(base, true)` (do not treat `dtstart` itself as the earliest
    start), falling back to the rule wall‑min when `dtstart` is absent. This
    removes environment‑dependent drift and preserves correct first occurrence
    selection (e.g., BYHOUR 05:00 with a midnight `dtstart`).
